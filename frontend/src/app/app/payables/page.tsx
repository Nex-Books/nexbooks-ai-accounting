...
import {
  MdDescription, MdRefresh, MdWarning, MdCheckCircle, MdUploadFile,
} from 'react-icons/md';
...
export default function PayablesPage() {
  ...
  filtered.filter(i => i.status !== 'paid').forEach(inv => {
    byVendor[inv.vendor_name] = (byVendor[inv.vendor_name] || 0) + inv.total_amount;
  });
  ...
  return (
    <Box bg={PAGE_BG} minH="100%" p={{ base: '16px', md: '28px' }}>
      ...
      <Flex justify="space-between" align="center" mb="24px" flexWrap="wrap" gap="12px">
        <Box>
          <Text fontSize="2xl" fontWeight="800" color={TEXT_DARK} letterSpacing="-0.6px">Accounts Payable</Text>
          <Text fontSize="sm" color={TEXT_MUTED} mt="2px">Track money you owe to vendors and suppliers</Text>
        </Box>
        <Flex gap="10px">
          <Button leftIcon={<MdRefresh />} onClick={fetchPayables} size="sm" variant="outline" borderRadius="10px" isLoading={loading}>Refresh</Button>
          <Link href="/app/invoices">
            <Button leftIcon={<MdUploadFile />} size="sm" colorScheme="purple" borderRadius="10px">Upload Bill</Button>
          </Link>
        </Flex>
      </Flex>
      ...
      <Grid templateColumns={{ base: '1fr', sm: 'repeat(3, 1fr)' }} gap="16px" mb="24px">
        {[
          { label: 'Total Outstanding', value: totalOutstanding, icon: MdDescription, bg: 'linear-gradient(135deg,#F2EFFF,#D5CCFF)', color: '#7551FF' },
          { label: 'Total Paid', value: totalPaid, icon: MdCheckCircle, bg: 'linear-gradient(135deg,#EFF4FB,#C7D7F4)', color: '#3965FF' },
          { label: 'Overdue Bills', value: overdue, icon: MdWarning, bg: 'linear-gradient(135deg,#FEEFEE,#FECACA)', color: '#EE5D50', isCount: true },
            ...
            <Flex align="center" gap="12px">
              ...
              <Box>
                ...
                {loading ? <Skeleton h="24px" w="80px" /> : (
                  <Text fontSize="xl" fontWeight="800" color={TEXT_DARK}>{isCount ? value : fmt(value)}</Text>
                )}
              </Box>
            </Flex>
          </Box>
      ...
      <Grid templateColumns={{ base: '1fr', lg: '1fr 1fr' }} gap="20px" mb="24px">
        <Box bg="white" borderRadius="16px" boxShadow={CARD_SHADOW} p="24px">
          ...
          <Text fontSize="xs" color={TEXT_MUTED} mb="20px">Outstanding payables by overdue period</Text>
          {loading ? <Skeleton h="200px" borderRadius="12px" /> : Object.keys(aging).length === 0 ? (
            <Flex direction="column" align="center" py="40px" gap="8px">
              <Icon as={MdCheckCircle} w="32px" h="32px" color="#01B574" />
              <Text fontSize="sm" color={TEXT_BODY}>No outstanding payables!</Text>
            </Flex>
          ) : (
            <Chart
              options={{
                chart: { type: 'bar', toolbar: { show: false }, fontFamily: 'DM Sans, sans-serif' },
                xaxis: { categories: Object.keys(aging), labels: { style: { colors: TEXT_MUTED } }, axisBorder: { show: false }, axisTicks: { show: false } },
              ...
              series={[{ name: 'Payable', data: Object.values(aging) }]}
        ...
        <Box bg="white" borderRadius="16px" boxShadow={CARD_SHADOW} p="24px">
          <Text fontWeight="700" fontSize="md" color={TEXT_DARK} mb="4px">Vendor-wise Outstanding</Text>
          <Text fontSize="xs" color={TEXT_MUTED} mb="20px">Top vendors by payable amount</Text>
          {loading ? [...Array(4)].map((_, i) => <Skeleton key={i} h="36px" mb="8px" borderRadius="8px" />) :
            topVendors.length === 0 ? <Text fontSize="sm" color={TEXT_BODY} textAlign="center" py="40px">No outstanding payables.</Text> : (
                ...
                {topVendors.map(([name, amount]) => (
                  <Box key={name}>
                    <Flex justify="space-between" mb="4px">
                      ...
                      <Text fontSize="sm" fontWeight="700" color="#7551FF" fontFamily="mono">{fmt(amount)}</Text>
                    </Flex>
                    <Box h="6px" bg="gray.100" borderRadius="full">
                      <Box h="6px" bg="#7551FF" borderRadius="full" w={`${(amount / maxVendorVal) * 100}%`} transition="width 0.5s" />
                    </Box>
                  </Box>
                ))}
              </Flex>
      ...
      <Box bg="white" borderRadius="16px" boxShadow={CARD_SHADOW} overflow="hidden">
        <Flex px="24px" pt="22px" pb="16px" gap="12px" flexWrap="wrap" align="center">
          <Text fontWeight="700" fontSize="md" color={TEXT_DARK} flex="1">Purchase Bills</Text>
          <Flex gap="10px" flexWrap="wrap">
            <Input placeholder="Search vendor / bill #" value={search} onChange={e => setSearch(e.target.value)} size="sm" borderRadius="10px" w="220px" />
        ...
        {loading ? (
          ...
          <Flex direction="column" align="center" py="60px" gap="12px">
            <Flex w="56px" h="56px" bg="purple.50" borderRadius="16px" align="center" justify="center">
              <Icon as={MdDescription} w="28px" h="28px" color="purple.400" />
            </Flex>
            <Text fontSize="sm" color={TEXT_BODY}>No purchase bills found.</Text>
            <Flex gap="8px">
              <Link href="/app/invoices">
                <Button size="sm" colorScheme="purple" borderRadius="10px">Upload Bill</Button>
              </Link>
          ...
          <Box overflowX="auto">
            <Table size="sm" variant="simple">
              <Thead>
                <Tr bg={PAGE_BG}>
                  {['Bill #', 'Vendor', 'Date', 'Due Date', 'Amount', 'Aging', 'Status'].map((h, i) => (
